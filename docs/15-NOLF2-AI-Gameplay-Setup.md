...
This document describes how the NOLF2 AI systems were used to set up basic gameplay. This is not an exhaustive description of every piece of the AI systems, but instead an overview of the most commonly used pieces, highlighting things that may not be obvious by looking at the code or DEdit properties.

In general, AI in NOLF2 are assigned either the Patrol, Guard, or Reinforcement `GoalSet`. There are a total of 46 `GoalSets` defined in `aigoals.txt`, but most are variations of Patrol or Guard for special characters such as ninjas, SuperSoldiers, and other special-case AI. See `aigoals.txt` for the full definitions.

Patrol, Guard, and Reinforcement share identical combat behavior, but vary in their relaxed and investigative behaviors. Below are descriptions of the expected behavior for each of these goal sets.

## Patrol, Guard, and Reinforcement GoalSets
...
- AI will walk along a patrol path defined by level design through `AINodePatrol` nodes.
...
- If no node is specified, the AI will find the nearest unowned `AINodePatrol` and take ownership of that node’s path.

- A patrolling AI may not own an `AINodeGuard`. Patrol and Guard are mutually exclusive.

- If a patrolling AI walks near an unowned work node, an `AINodeUseObject` with `SmartObject` set to `Work`, it will stop patrolling to do some work and then continue patrolling. Patrolling AI will ignore work nodes owned by an `AINodeGuard`.
...
- If no node is specified, the AI will find the nearest unowned `AINodeGuard`.

- A guarding AI may not own any `AINodePatrol` nodes. Patrol and Guard are mutually exclusive.
...
- Menace nodes are used exactly like work nodes, but in levels where the AI is supposed to already be aware of the enemy’s presence and should appear permanently agitated even when the AI system considers it relaxed. In other words, when the AI is not in combat it will run between menace nodes, `AINodeUseObject` nodes with `SmartObject` set to `Menace`, and play aggressive animations.
...
- Reinforcement relaxed behaviors are defined by the goals Guard, Work, ExitLevel, and Menace. See the Guard goal set above for more information on Menace.
...
- If no unclaimed nodes are found, the AI will look for an `AINodeExitLevel`. If an ExitLevel node is found, the AI will walk to the node and be removed from the level. These are normally placed in areas the player cannot reach, so it is not obvious that the AI is being instantly removed from the game.

- The AI will not look for an ExitLevel node until it has been in the level for at least 5 seconds. This restriction prevents an AI from spawning near an ExitLevel node and immediately removing itself before noticing an alarm or other stimuli.
