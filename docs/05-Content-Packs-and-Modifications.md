...
## Matching the Game Directory Structure
...
When you installed the Nolf 2 tools, you were prompted to un-rez the Nolf 2 resources. If you chose yes at this prompt, then the installer called `LithRez.exe` to unpack all of the Nolf 2 resources into a directory structure that can be recognized by DEdit and other tools. You can also unpack rez files manually from the command prompt by browsing to the `Tools/Bin` folder and typing the following:

`lithrez x RezFileName.rez TargetFolderName`

The NOLF2 project (to2.dep) looks for resources in the Game subfolder of your main Nolf 2 installation directory. Therefore, files unpacked into any other folder will not show up in DEdit. Let's say, for example,, you downloaded a .rez file called “NewDMMaps.rez” from the Internet that included the source .ltc level files, and that you saved the file to C:\Downloads. To extract the resources into their proper NOLF2 folders (assuming that the .rez file was created correctly), you would browse to the Tools/Bin folder and type:
...
## Launcher and Dedicated Server Source
...
## Custom Map Names and Preview Images
...
1. Copy the file to a valid texture directory such as `game\tex`.
2. With the NOLF2 project loaded, click the **Textures** tab.
3. Right-click anywhere in the lower pane.
4. Select **Import TGA Files** or the equivalent import option for your source format.
5. Browse to the folder that you copied the file to.
6. Select the TGA file you want to import.
7. Click **Open**.
8. Left-click the newly imported TGA file and select **Texture Properties**.
9. Change the compression setting from 32-bit to `S3TC DTX1 (8x compression)`.
...
Asset lists contain the filenames of resources that are used by a level and its components during a game, but are not automatically loaded at the start of the level. When precaching is turned on in the options menu, NOLF2 will look for an assetlist for the level after it is loaded, and then precache all of the resources in the list. This greatly reduces hitching in the level.

Before you can create asset lists, you will need to install the Perl programming language. You can download and install ActivePerl free of charge at [http://www.activestate.com/Products/ActivePerl](http://www.activestate.com/Products/ActivePerl). Once this is installed, you can create asset lists like this:

1. In the advanced options menu in the NOLF 2 launcher, enter the following on the command line:

   `errorlog 1 alwaysflushlog 1 showfileaccess 1`

2. Run the level you want to build an asset list for. Do not use cheats, and try to exercise everything available in the level, including conversations, secrets, weapons, ammo types, explosions, debris, and HUD options.
3. When you are confident that you have triggered everything that may dynamically load an asset, quit the game without loading any additional levels.
4. Rename `error.log` to the exact name of the level with a `.log` extension, for example `mylevel.log`.
5. Open the log file in a text editor and delete:
   Delete everything before the line that reads something like `Loaded world: Worlds\RetailSinglePlayer\mylevel in 3.87 seconds`.
   Delete everything after the line `World Ended`.
6. Save your changes and exit the text editor.
7. If you need multiple playthroughs to capture every asset, repeat steps 1 through 6 and rename each new log accordingly. Then merge the contents into a single `mylevel.log`. The ordering does not matter as long as the complete list of resources is present.
8. Copy the edited log file to `Tools\AssetLists\logs\new`.
9. Double-click `TO2\Development\Tools\AssetLists\BuildAssetLists.pl`. When you run this script, the log file in `AssetLists\logs\new` will be moved to `AssetLists\logs\processed` and renamed based on the time and date. The log will then be processed, an intermediate file will be copied to `AssetLists\processing`, and the final `.txt` version will be copied to `AssetLists\final`.
10. Copy `AssetLists\Final\levelname.txt` into the `Game\Worlds\` directory associated with the level that was logged. For example, if the level was `Game\Worlds\RetailMultiPlayer\dm_mylevel.dat`, copy `AssetLists\Final\dm_mylevel.txt` to `Game\Worlds\RetailMultiPlayer`.
...
In order to add cooperative missions to No One Lives Forever 2, you must make a mod rather than just a content pack. The list of cooperative missions used by the game is stored in an attribute file:

`Attributes\CoopMissions.txt`
...
- `NameId` specifies the string ID of the mission name. See the adding strings section above for more details.
...
- `Level0` is the path to the first level of the mission. A mission may have up to 10 levels. `Level1` specifies the second level, `Level2` the third, and so on.
- `Level0BriefingId` is the string ID for briefing text for the first level of the mission. If a level does not require a briefing, this entry may be omitted. A mission may have up to 10 levels, so `Level1BriefingId`, `Level2BriefingId`, and so on follow the same pattern.
