...
![textureartist002](images/textureartist002.jpg)
...
This changes the effect group parameter to the specified value. `Stage` represents the stage being changed and can be either `1` or `2`. It cannot refer to a stage that is disabled or overridden. `Variable` represents the index of the variable to change and corresponds to the order in which the variables are listed in DEdit, from left to right and top to bottom, and must be from one to six. `Value` is the value stored at that location.

For example, if we had a panning script with the parameters `DirX`, `DirY`, `DirZ`, `Velocity`, and `Offset`, and that script was part of the effect group `ConveyorBelt.tfg`, we would first create a `TextureFX` object named `ConveyorBelt` whose `TextureEffect` parameter was assigned to `ConveyorBelt.tfg`. We could then set up a trigger in a doorway that starts the conveyor belt by changing its velocity from `0` to `5`. The conveyor belt uses stage one, and since velocity is the fourth parameter, the message would look like this:

*Msg ConveyorBelt SETVAR (1 4 5.0)*

Below is a diagram that illustrates the process of creating texture effects:

![textureartist006](images/textureartist006.gif)
